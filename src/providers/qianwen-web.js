...
export const qianwenWebProfile = {
  ...
  quota: {
    ...
    promptsPerWindow: 1,
    // 匿名额度是**按轮**给的，不是按天。2026-09-23/24 四次运行形状完全一致：批次 58 成功 3 条、
    // 批次 60/63/65 各成功 4 条，然后第 5 条撞登录墙；墙**只在完全安静 20–30 分钟后**才解除
    // （批次 63 连续敲了 6 小时，墙一直没退；停掉静置后再探一条就成功）。而且撞墙那一条已经
    // 送进对话、救不回来，所以"失败后冷却"这个思路没用 —— 失败发生时就晚了。
