 ...
 *
 * 180s 这个值来自**实测**，不再是推理值。2026-09-26 重跑批次 68 时每条成功采集都会打一条
 * `[qianwen] first-token`，7 条样本的首字延迟是：
 *
 *   8s, 8s, 8s, 8s, 6s, 50s, 97s
 *
 * —— 中位数 8s，但**尾部到了 97s**。最初按推理定的 120s 只比它高 23s，太紧了：这个判定的
 * 代价不可逆（被判死的那条提问已经提交，重跑就是重复提问），所以取 97s 的约 1.85 倍。
 * 定到 180s 的代价是零输出任务白等 3 分钟，而原来要白等满 15 分钟 —— 依然是 5 倍差距。
 * 样本只有 7 条，后续用同一条 `first-token` 日志继续校准；分布尾部一旦越过它，就该再抬。
 *
 * 判据只认「从未出现过答案」（firstTokenSeen），一旦出过一个字就永久关闭 ——
