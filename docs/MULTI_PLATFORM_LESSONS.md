...
- **60 秒都不够**：修完上面两条之后仍抓到一条只有 **15 个字符**的"成功"记录（正文 `在绍兴越城区想找手法好的推拿店`），说明平台的静止期可以超过一分钟。想靠"文本安静"收尾，窗口得开到数分钟（100 条就要 7–10 小时）。**正解是改用平台自己的"回答已完成"标记**（消息操作按钮 / 落款），而不是继续加长猜测窗口。
